...
class Company {
  ...
  static async findAllFiltered(data) {
    ...
    const minEmployees = data.minEmployees || 0;
    const maxEmployees = data.maxEmployees || Infinity;

    if (minEmployees > maxEmployees) {
      throw new BadRequestError(`Minimum employees (${minEmployees}) must be lower than maximum eployees (${maxEmployees})`);
    }
    ...
    if (!name) {
      ...
      querySql = `
                      ...
                      FROM companies
                      WHERE name ILIKE '%'||$1||'%'`;
      result = await db.query(querySql, [name]);
    }
